...
export function ProductForm({
  ...
  // Código interno que está GRAVADO no banco — a etiqueta só sai com ele, nunca
  // com o que está digitado/gerado no campo e ainda não foi salvo.
  const [savedInternalCode, setSavedInternalCode] = useState(defaultValues?.internalCode ?? "");
  ...
  function handleOpenLabel() {
    if (!productId) return;
    const typedCode = (watch("internalCode") ?? "").trim();
    if (typedCode !== savedInternalCode) {
      setServerError(
        "O código interno foi alterado e ainda não foi salvo. Clique em \"Salvar produto\" e depois imprima a etiqueta pela lista de produtos."
      );
      return;
    }
    setServerError(undefined);
    setLabelTarget({ id: productId, name: watch("name"), internalCode: savedInternalCode || null });
  }
  ...
  return (
    <>
            ...
            <div className="flex gap-2">
              ...
              {productId && (
                <Button
                  ...
                  title="Imprimir etiqueta do código interno"
                >
      ...
      {/* Fora do <form>: Enter nos campos da etiqueta não pode enviar o cadastro. */}
      <ProductLabelDialog
        target={labelTarget}
        onClose={() => setLabelTarget(null)}
        onCodeGenerated={(code) => {
          // Já está gravado no banco; espelha no campo pra um "Salvar produto"
          // depois não apagar o código que acabou de ir pra etiqueta.
          setSavedInternalCode(code);
          setValue("internalCode", code, { shouldValidate: true });
        }}
      />
    </>
  );
